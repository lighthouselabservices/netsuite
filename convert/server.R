...
shinyServer(function(input, output, session) {
  ...
  output$dt_input <- renderDT({
    ...
    # make datatable
    dat_view<-DT::datatable(dt_in, rownames=FALSE, style = "default", class = "display compact", selection = "single",  filter = 'top',
  ...
  output$dt_fields <- renderDT({
    ...
    # make datatable
    dat_view<-DT::datatable(dt_names, caption = "Input Table Field Names" , rownames=FALSE, style = "default", class = "display compact", selection = "single",  filter = 'top',
  ...
  config_yaml <- reactive({ config_options[input$type] })
  ...
  output$dt_output <- renderDT({
    ...
    # make datatable
    dat_view<-DT::datatable(dt_out, rownames=FALSE, style = "default", class = "display compact", selection = "single",  filter = 'top',
                            options=list(dom='ipft', deferRender=TRUE, width = '100%',  pageLength = 20 ))
